...
-- Gọi thủ tục trên với giá trị id user và p_bonus_points bất kì mà bạn muốn cập nhật
set @bonus_points = 100;
call calculatebonuspoints(2, @bonus_points);
...
-- GIỎI 4
--  Viết procedure tên CreatePostWithValidation nhận IN p_user_id (INT), IN p_content (TEXT).
delimiter $$
...
call createpostwithvalidation(1, 'hi', @result_message); -- trường hợp 1: nội dung quá ngắn (< 5 ký tự)
call createpostwithvalidation(1, 'day la bai viet hop le', @result_message); -- trường hợp 2: nội dung hợp lệ
...
-- XUẤT SẮC 5
-- Viết procedure tên CalculateUserActivityScore nhận IN p_user_id (INT), trả về OUT activity_score (INT).
delimiter $$
...
begin
    ...
    -- Đếm số like nhận được bài viết của user
    select count(*)
    into like_count
    from likes l
    join posts p on l.post_id = p.post_id
...
--  Gọi thủ tục trên select ra activity_score và activity_level
...
begin
    ...
    -- Cursor lấy danh sách bạn bè (2 chiều)
    ...
    -- Lấy full name của người đăng bài
    select full_name
    into v_full_name
    from users
    where user_id = p_user_id;
    ...
    friend_loop: loop
        ...
        -- Không gửi thông báo cho chính người đăng
    ...
    -- Trả về post_id vừa tạo tiện kiểm tra
    select v_post_id as new_post_id;
end $$
delimiter ;

-- Gọi procedue trên và thêm bài viết mới 
call notifyfriendsonnewpost(1, 'hom nay minh vua dang bai viet moi');
